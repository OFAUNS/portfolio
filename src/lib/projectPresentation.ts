...
const presentations: Record<string, ProjectPresentation> = {
    ...
    motion: {
        ...
        mediaTreatment: "loop",
        imageFit: "cover",
        summary: "AE 圓形動畫試作。圓形是主題、循環是動畫的本質——定格沿著一圈排開，走完一圈就是一個 loop。",
        narrative:
            "圓形動畫的接觸印樣不該排成方格，它本來就是循環的。七張定格繞成一圈，影片在頁首直接 loop，底下補一條 easing 曲線——節拍不是等速，那條曲線才是動畫真正的內容。",
        galleryTitle: "一圈 · 七張定格",
        contentLabel: "One Loop",
        homeIntro: "定格排成一整圈，走完一圈就是一個 loop，底下用 easing 曲線講節拍。",
        focus: [
            {
                title: "形狀就是主題",
                text: "圓形是這支試作唯一用到的圖形，所以整個版面的骨架也是圓——形式跟內容用同一個形狀講完。",
            },
            {
                title: "循環是動畫的本質",
                text: "起點與終點在同一個位置，讀者的視線繞完一圈就等於看完一次 loop。",
            },
            {
                title: "這是練習不是完成作",
                text: "只有七張定格、一段 2.4 秒的循環，版面不放大成大作品，誠實呈現它的量。",
            },
        ],
    },
    film: {
